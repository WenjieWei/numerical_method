...
\begin{document}
	...
	\section{Coaxial Cable Electrostatic Problem}
		...
		\section{Conjugate Gradient}
			Listing \ref{lst:conjugate_gradient} shows the implementation of the conjugate gradient method. The important step of this problem is to find the matrix \textbf{\textit{A}} and the vector \textbf{\textit{b}} used for Choleski decomposition and conjugate gradient.
			
			For this particular problem, using a five-point difference model discussed before, the \textbf{\textit{A}} matrix is constructed for the free nodes lying between the conductors. Each free node is represented by a row in the \textbf{\textit{A}} matrix. The \textbf{\textit{b}} vector is composed of the fixed voltage values directly neighboring the free nodes, either 0 or -110$V$.
			...
			\subsection{$\infty$ Norm and 2-Norm}
				\begin{figure}[H]
					...
					\caption{Plot of $2y$-norm v.s. iterations}
					\label{two_norm_plot}
				\end{figure}	
			...
			\subsection{Computation of Capacitance per Unit Length}
				...
				\begin{equation}
					E = \frac{1}{2} \int_{V} \varepsilon_0E^2dv
				\end{equation}
